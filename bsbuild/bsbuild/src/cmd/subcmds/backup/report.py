...
class DiffManifestFile:
    ...
    def savereportas(self, path):

        reportlines = []
        conflictlines = []

        totalconflictpatches = 0

        reportlines.append("idx,stat,hash,title,time,author,confirm\n")
        for p in self.sections[SECTION_CHANGED].projects:
            if p.groups is not None and 'miui' in p.groups:
                conflictlines = []
                idx = 0
                for pth in p.patches:
                    if OP_FLAG_CONFLICTED in pth.opflag:
                        totalconflictpatches += 1
                        idx += 1
                        conflictlines.append(
                            "%d,conflict,%s,%s,%s,%s\n" %
                            (idx, pth.hash, pth.title.replace(',', r'，'), pth.time, pth.author))

                if len(conflictlines) > 0:
                    reportlines.append("%s\n" % (p.path))
                    reportlines.extend(conflictlines)

        reportlines.append("total: %d" % (totalconflictpatches))
...
def main():
    global gSourceDiffManifest
    gSourceDiffManifest = DiffManifestFile('diffmanifests_gen_2019_10_28__13_11_25.txt', 'r+');
    ...
    gSourceDiffManifest.savereportas('diffmanifests_gen_%s.csv' % nowStr)
